...
`Invoke-ReviewedMigration.ps1` requires `ExpectedMachineGuid` and intentionally supports only a local SQL Server host or instance and a local absolute backup path. It verifies the copy-only backup before applying the migration. The SQL Server service account must have write access to the local backup directory before the migration is attempted.
...
The rollout script reports `Applied`, `WhatIf`, or `Declined` status and does not implement rollback automatically. Restoring the pre-migration backup and revalidating application readiness remains the supported rollback boundary. Phase 9 must still rehearse forward migration, restore, retention, and verification against a representative topology.
