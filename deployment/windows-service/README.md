...
- `Install-WindowsScriptRunnerWorker.ps1` with `ExpectedMachineGuid`, explicit service identity, publish-root validation, automatic restart actions, ACL setup, upgrade protection, post-install verification, and `-WhatIf` support; and
- `Verify-WindowsScriptRunnerWorker.ps1` with required `ExpectedServiceAccount` for exact executable, account, automatic-start, and optional running-state checks.
