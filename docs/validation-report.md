...
Initial validation date: 2026-07-31. Times are America/Chicago (`-05:00`). The initial local validation preceded the review-branch commit and pull request; deployment and production rollout remain out of scope.
...
- Application calculates the `windows-script-runner-approval-v2` SHA-256 fingerprint from current trusted job/script/policy/target/parameter data and immutable accepted DryRun evidence. The evidence binds the DryRun work kind, source, worker/lease/fencing provenance when leased, and execution-window timestamps; Execute attempts cannot substitute for it. Decision handlers obtain the authenticated actor from `ICurrentUser`, recompute evidence, compare it in constant time, then preserve Domain separation-of-duties and the existing audit/unit-of-work boundary.
- Focused Web test-host coverage passed 8/8. It covers anonymous challenge, rendered access-denied behavior, policy/resource matrix, list/lookup/detail filtering, safe rendered report surface, antiforgery, forged form fields, authenticated actor, POST-Redirect-GET, replay safety, and self-approval conflict. Focused identity/resource/Web coverage passed 47/47, including the real current Windows token group fallback.
...
- Follow-up PR review remediation makes the unprivileged report list query only the authenticated requester's reports before applying its 100-result bound; report-reader, approver, and administrator mappings retain the bounded all-report view. The accepted-DryRun-evidence constraint now fails closed for SQL `UNKNOWN`, partial, and empty-identifier states. The upgrade migration cancels every legacy Execute state after DryRun completion but before an execution attempt (`DryRunCompleted`, `AwaitingApproval`, `Approved`, `ExecutionQueued`, and `Claimed`), removes a legacy claimed lease, and writes system audit events instead of fabricating evidence. Domain requires the same trusted evidence and canonical requester before queueing, leasing, or starting Execute work. Web-only actor/fingerprint handlers register only in the Web composition root, leaving the Worker container independent of `ICurrentUser`. Draft creation now takes `RequestedBy` exclusively from trusted `ICurrentUser`, and Domain refuses a non-canonical requester before Execute approval or rejection; a two-requester portal test proves report-list isolation and the authenticated navigation remains discoverable. Completed legacy records with a non-SID requester remain group-reader-only because Phase 8 refuses unsafe name-to-SID ownership inference; controlled offline reconciliation is deferred. Empty portal identifiers return 404 before strong-ID construction, malformed report lookup identifiers return 400 before any list query, and the report-list index matches the `(ReportType ASC, CreatedUtc DESC, Id ASC)` query ordering. The approval view renders the bounded pinned script identity/version/SHA-256, captured risk, requested phase, and accepted DryRun window before it presents decision buttons, without rendering paths, raw output, secrets, or worker/lease/fencing provenance. SQL, Application, Worker, and Web tests cover the scoped query, upgrade, execution guard, identity boundary, review context, composition boundary, empty/malformed ID handling, index direction, and 403/404 portal paths. The database-migration and approval-fingerprint documents now describe the required evidence migration and v2 canonical evidence precisely.
...
- `dotnet restore`: exit 0; all projects up to date.
- `dotnet build --configuration Release`: exit 0; 0 warnings and 0 errors.
- `dotnet test --configuration Release`: exit 0; 739 passed, 0 failed, 0 skipped: Unit 408, Security 110, SQL Server 56, Worker 52, PowerShell boundary 110, Integration 3.
...
The review remediation adds EF migration `20260731165153_AddTrustedDryRunApprovalEvidence`, which persists accepted DryRun evidence and creates the bounded report-list index. Residual risks intentionally deferred include audited offline reconciliation of pre-Phase-8 non-SID requester ownership, IIS/HTTPS configuration, Windows service installation, SPN/Kerberos/browser-zone/delegation validation, service identities, production SQL rollout and rollback, backup/restore rehearsal, secret integration, observability, operational runbooks, and production hosting verification. Existing Phase 5 process-containment, filesystem TOCTOU, lack-of-OS-sandbox, and at-least-once execution limitations remain unchanged.
