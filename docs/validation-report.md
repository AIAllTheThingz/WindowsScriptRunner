...
- SQL Server focused suite: 41 passed, 0 failed, 0 skipped against SQL Server LocalDB. Multi-worker coverage includes two-worker/one-job and four-worker/30-job races with unique ownership/fencing. SQL Server UTC coordination and expired-lease discovery use the shared database clock. Renewal/recovery and duplicate-recovery races each produce one valid winner. Active DryRun/Execute/PostValidation recovery and stale completion rejection passed.
