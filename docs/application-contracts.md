...
Serialized `StringArray` parameter values use a JSON array of strings, for example `["server-01","server-02"]`. This representation is validated in Domain. The current reviewed local-host inventory package accepts no parameters. `SecureReference` values must use canonical GUID `D` format and represent `CredentialReferenceId`; arbitrary strings and raw secret-shaped values are rejected before storage.
...
`IJobQueueCandidateSource` accepts the exact supported `JobWorkRoute` set, a bounded count, and the current time. Each route is a `JobWorkKind` plus a pinned `ScriptVersionId`; this prevents a worker from claiming an unsupported script merely because the work kind matches. Candidates return only `JobId`, `JobWorkKind`, `ScriptVersionId`, `CreatedUtc`, and `UpdatedUtc`. `IExpiredJobLeaseCandidateSource` returns bounded expired lease identifiers and fenced credentials. `IFencingTokenSource` supplies a positive monotonic token from SQL Server.
...
`ClaimedJobWork` contains only the job/work identifiers, pinned script-version ID, worker/lease identifiers, fencing token, and expiration. It never contains parameters, credential-reference IDs, script paths, hashes, or report output. Each `IJobWorkHandler` advertises its exact supported routes and receives the claimed descriptor plus cancellation. A successful handler must explicitly resolve its lease through a lifecycle completion or safe unstarted release; merely returning is an invariant violation. For the reviewed local-host inventory route, a zero process exit code is not success until Reporting validates and canonicalizes the bounded output.
