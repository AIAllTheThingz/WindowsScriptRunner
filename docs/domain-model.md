...
classDiagram
    ...
    Job "1" *-- "*" JobApproval
    Job "1" *-- "0..1" JobReport
    JobReport "1" *-- "1" LocalHostInventoryReportPayload
...
`JobReport` is immutable, belongs to one job and pinned script version, records a known `ReportFormat`, and owns exactly one typed payload. The current `LocalHostInventoryReportPayload` contains only canonical, bounded report fields. Domain construction and rehydration reject missing, duplicate, mismatched, undefined, or malformed report state.

Strong GUID-backed IDs are immutable reference records with no public parameterless constructor. Constructors reject `Guid.Empty`, factories use explicit `New()` methods, and aggregate/entity boundaries reject null IDs. `ScriptName`, `ScriptVersionNumber`, `UserIdentity`, `TargetName`, `ChangeReference`, and `WorkerCapability` centralize validation and equality. `UserIdentity` preserves display casing but uses ordinal case-insensitive equality and hash codes. Phase 8 maps authenticated Windows principals to canonical `sid:<windows-sid>` values at the Web boundary; Domain remains independent of ASP.NET and does not parse claims.

The Domain project owns no persistence, ASP.NET, SQL, PowerShell, file-system, Git, or serialization concerns. Persistence entities and mappings stay in Infrastructure. Domain aggregates expose internal validated rehydration factories to Infrastructure through `InternalsVisibleTo`; they do not expose public persistence constructors, public setters, or EF attributes. Rehydration rejects inconsistent identifiers, undefined enums, invalid timestamps, duplicate children, incomplete executions, missing typed report details, and other corrupt stored state. Persisted `JobParameter` rows remain binding-only, and the pinned immutable script version stays authoritative before any value is exposed.
