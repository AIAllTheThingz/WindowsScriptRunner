...
`Job` owns its optional `JobLease`. Application handlers validate the worker's enabled/live state, obtain SQL Server UTC through the scoped worker-coordination clock, obtain a monotonic SQL fencing token, mutate the aggregate, stage bounded audit metadata, and commit once. Registration, heartbeat, acquisition, renewal, leased lifecycle validation, and expiration recovery therefore share one time authority across hosts. The queue tracks every dispatch, renews through fresh scopes, cancels work after lease loss, limits local concurrency, and drains for a configured interval at shutdown. Expired leases are independently revalidated and recovered. This produces at-least-once coordination; it does not promise exactly-once external side effects.
