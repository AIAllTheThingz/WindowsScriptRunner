...
At submission, `Job` captures the trusted script-definition ID, version ID, risk, Execute support, and PostValidation support in an immutable policy snapshot after validating that the risk and requested phase are defined enum values. Later approval, read-only completion, and post-validation entry decisions use this snapshot rather than caller-supplied policy values. Infrastructure persists and reconstructs this snapshot through validated mappings.
...
Subsequent status: the reviewed package and typed report both revalidate the exact pinned published definition/version. Phase 8 will add the authenticated approval boundary and trusted fingerprint calculation.
