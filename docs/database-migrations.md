...
`20260731165153_AddTrustedDryRunApprovalEvidence` persists the immutable accepted DryRun evidence required by Phase 8 approval fingerprints. It adds source, trusted lease/worker/fencing provenance, and execution-window timestamp fields to `wsr.Jobs`, with database constraints that reject incomplete, inconsistent, or non-DryRun evidence. It also adds the bounded Local Host Inventory report-list index `(ReportType, CreatedUtc, Id)`.

Applying the migration does not backfill accepted DryRun evidence. To avoid fabricating approval evidence, legacy Execute jobs in `DryRunCompleted` or `AwaitingApproval` are changed to `Cancelled` and receive a `LegacyDryRunEvidenceUnavailable` system audit event. Operators must create a new request and obtain a fresh DryRun before approval. The structural down migration removes the evidence columns/constraint and report-list index; it does not restore terminally cancelled legacy jobs. Production rollback therefore requires a reviewed backup, maintenance-window, and revalidation plan. The pending-model validation command remains required and must report no changes.
