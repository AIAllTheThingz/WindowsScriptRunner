...
Applying the migration does not backfill accepted DryRun evidence: jobs that were already awaiting approval must be reviewed through the existing lifecycle and cannot use approval fingerprints until they have trusted evidence. The structural down migration removes the evidence columns/constraint and report-list index. It consequently destroys the accepted-evidence record needed for any subsequent approval decision, so production rollback requires a reviewed backup, maintenance-window, and revalidation plan. The pending-model validation command remains required and must report no changes.
